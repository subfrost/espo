...
//!
//! Exercises pool discovery, activity tracking, and queries through
//! the real devnet pipeline with AMM contract deployment.
...
mod tests {
    ...
    fn create_modules() -> Result<(Essentials, AmmData, Arc<DB>, tempfile::TempDir)> {
        let temp_dir = tempfile::tempdir()?;
        ...
        let amm_mdb = Arc::new(Mdb::from_db(db.clone(), b"ammdata:"));
        let mut ammdata = AmmData::new();
        ammdata.set_mdb(amm_mdb);
        // Set ammdata config
        ammdata.set_config(&serde_json::json!({
            "eth_rpc": "http://127.0.0.1:8545",
            "eth_call_throttle": 0,
            "search_index_enabled": false
        }))?;

        Ok((essentials, ammdata, db, temp_dir))
    }
    ...
    #[test]
    fn test_ammdata_indexes_setup_blocks() -> Result<()> {
        let rt = tokio::runtime::Runtime::new()?;
        let _guard = rt.enter();
        init_test_config()?;

        let metashrew = TestMetashrewRuntime::new()?;
        let (essentials, ammdata, _db, _tmp) = create_modules()?;

        setup_and_index(&metashrew, &essentials, &ammdata, 5)?;

        assert_eq!(ammdata.get_index_height(), Some(4));
        println!("[AMMDATA] 5 setup blocks indexed OK");
        Ok(())
    }

    #[test]
    fn test_ammdata_with_amm_deployment() -> Result<()> {
        let rt = tokio::runtime::Runtime::new()?;
        let _guard = rt.enter();
        init_test_config()?;
        ...
        setup_and_index(&metashrew, &essentials, &ammdata, 4)?;
        ...
        assert_eq!(ammdata.get_index_height(), Some(end));
        println!("[AMMDATA] AMM deployment indexed through height {end}: OK");
        Ok(())
    }

    #[test]
    fn test_ammdata_genesis_block_regtest() {
        let ammdata = AmmData::new();
        assert_eq!(ammdata.get_genesis_block(bitcoin::Network::Regtest), 0);
    }
}
