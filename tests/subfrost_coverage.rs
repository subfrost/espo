...
//! Integration tests for the subfrost module.
//!
//! Exercises wrap/unwrap event detection and storage through the real devnet.
...
mod tests {
    ...
    fn init_test_config() -> Result<()> {
        ...
        let config = espo::config::AppConfig {
            ...
            modules: {
                ...
                m.insert("subfrost".to_string(), serde_json::json!({}));
                m
            },
        };
    ...
    fn create_modules() -> Result<(Essentials, Subfrost, Arc<DB>, tempfile::TempDir)> {
        let temp_dir = tempfile::tempdir()?;
        ...
        let sub_mdb = Arc::new(Mdb::from_db(db.clone(), b"subfrost:"));
        let mut subfrost = Subfrost::new();
        subfrost.set_mdb(sub_mdb);

        Ok((essentials, subfrost, db, temp_dir))
    }

    // ========================================================================

    #[test]
    fn test_subfrost_indexes_setup_blocks() -> Result<()> {
        let rt = tokio::runtime::Runtime::new()?;
        let _guard = rt.enter();
        init_test_config()?;

        let metashrew = TestMetashrewRuntime::new()?;
        let (essentials, subfrost, _db, _tmp) = create_modules()?;
        ...
        for h in 0..5 {
            ...
            subfrost.index_block(espo_block)?;
        }

        assert_eq!(subfrost.get_index_height(), Some(4));
        println!("[SUBFROST] 5 setup blocks indexed OK");
        Ok(())
    }

    #[test]
    fn test_subfrost_with_amm_deployment() -> Result<()> {
        let rt = tokio::runtime::Runtime::new()?;
        let _guard = rt.enter();
        init_test_config()?;
        ...
        // Deploy AMM (no subfrost events, but should index without error)
        ...
        assert_eq!(subfrost.get_index_height(), Some(end));
        println!("[SUBFROST] AMM deployment indexed (no subfrost events expected): OK");
        Ok(())
    }

    #[test]
    fn test_subfrost_genesis_block() {
        let subfrost = Subfrost::new();
        assert_eq!(subfrost.get_genesis_block(bitcoin::Network::Regtest), 0);
        assert_eq!(subfrost.get_genesis_block(bitcoin::Network::Bitcoin), 0);
    }
}
