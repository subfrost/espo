...
3. If `type` is not provided:
   - If Option A exists, page `address_amm_history` and fetch activity entries.
   - Otherwise, fetch N entries from each address index and merge by timestamp.
...
3. Build `TokenPair` entries with:
   - `poolId`, `poolName`, `poolTvlInUsd`, `poolVolume1dInUsd`
...
5. Map to `TokenPair` with metadata and metrics.
